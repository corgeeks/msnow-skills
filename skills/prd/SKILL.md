...
Before running any script, resolve its full path from this skill's directory — e.g. if this file loaded from `/abs/path/to/skills/prd/SKILL.md`, then `scripts/list-prds.sh` means `/abs/path/to/skills/prd/scripts/list-prds.sh`. All script references in the workflows use these skill-relative paths. The scripts themselves operate on PRDs stored at `<prd-root>/<prd-name>/` **relative to the current working directory** (the user's project root).

**The PRD root is not everyone's `.claude/`.** Not every collaborator on a project uses Claude, so PRDs — which are product documentation, not Claude-specific tooling state — default to a plain, visible `docs/prd/` rather than being hidden inside `.claude/`. Every script resolves this the same way (`scripts/lib/prd-root.sh`): an explicit `PRD_ROOT_DIR` env var, then a project's `.claude/prd-root` config file (a previously-chosen path, one line), then a pre-existing `.claude/prds/` directory (so old installs of this skill keep working untouched), then the `docs/prd` default. You never need to pass the root explicitly — resolve it once (see CreatePRD's "Determine the PRD Root Directory" step, which prompts the user only the first time a project has no PRDs yet) and it stays consistent for every later script call and session. `scripts/prd-root.sh` prints the resolved value if you need it directly.
